...
let companionClientSuite = TestSuite("CompanionClient", [
    ...
    TestCase("createManualMemory posts body + decodes response") {
        ...
        let req = ManualMemoryRequest(text: "Hello Continuum", clientEventId: "evt-1", captureType: "idea")
        ...
        try expect(sent.contains("\"text\":\"Hello Continuum\""))
    },
    ...
    TestCase("ask() posts query and decodes source cards") {
        let body = #"""
        {
          ...
          "answer":"You were shipping ContinuumKit.",
          "verify_outcome":"grounded",
          "source_cards":[
            {
              "memory_id":"mem_1",
              "title":"ContinuumKit",
              "summary":"Worked on ContinuumKit.",
              "display_summary":"Worked on ContinuumKit.",
        ...
        try expectEqual(response.answer, "You were shipping ContinuumKit.")
    ...
    TestCase("searchMemories() decodes cards and total") {
        ...
        let response = try await client.searchMemories(request: MemorySearchRequest(query: "continuum", limit: 10))
        try expectEqual(response.query, "continuum")
