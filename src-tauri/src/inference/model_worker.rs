...
impl QwenVlmWorker {
    ...
    pub fn new(app_data_dir: std::path::PathBuf) -> Arc<Self> {
        use crate::inference::model_config::{
            MAX_CONCURRENT_MULTIMODAL_JOBS, QWEN_IDLE_UNLOAD_SECONDS,
        };
    ...
    pub async fn synthesize(&self, input: MemorySynthesisInput) -> SynthesisResult {
        ...
        reply_rx
            .await
            .map_err(|_| "qwen_worker_dropped".to_string())?
    }
...
pub(crate) async fn run_synthesis_blocking(
    ...
    if let Some(ref img_path) = input.image_path {
        if let Err(e) = tokio::fs::remove_file(img_path).await {
            tracing::warn!(
                "qwen_worker: failed to delete temp screenshot {:?}: {e}",
                img_path
            );
        } else {
...
pub fn decide_model_run(
    ...
    if is_duplicate {
        return ModelRunDecision::Defer {
            reason: "duplicate".to_string(),
        };
    }
    if system_pressure_skip {
        return ModelRunDecision::OcrOnlyFallback {
...
mod tests {
    ...
    async fn temp_screenshot_deleted_after_inference() {
        ...
        use crate::inference::qwen_vl_memory::{MemorySourceType, MemorySynthesisInput};
        ...
        assert!(
            !img_path.exists(),
            "temp screenshot must be deleted after inference"
        );
    }
}
