...
use fndr_lib::context_runtime::context_pack::{
    EvidencePack, FusedHit, FusionSignals, FusionWeights, SurfacingReason, VerifyOutcome,
};
use fndr_lib::context_runtime::evidence_pack::collect_evidence;
use fndr_lib::context_runtime::fusion::fuse;
use fndr_lib::context_runtime::query_plan::{plan, PlanHints, Route};
use fndr_lib::context_runtime::retrieval_routes::{RouteCtx, RouteRunner};
use fndr_lib::context_runtime::verifier::verify;
...
fn record(id: &str, text: &str, timestamp: i64, embedding: Vec<f32>) -> MemoryRecord {
    MemoryRecord {
        ...
        window_title: "FNDR planner debounce".to_string(),
...
fn run_query_returns_grounded_answer_with_evidence_for_planner_bug() {
    ...
    let texts = [
        "FNDR planner debounce fix landed in plan.ts after the alpha bug report",
