...
void main(){
  ...
  vec3 col = mix(uBg, uMid, glow);
  col = mix(col, uAcc, pop * 0.62);
  col = mix(col, uAcc, s * 0.42);
  col += uAcc * mouseGlow * 0.12;
...
void main(){
  ...
  vec3 col = mix(uBg, uMid, cloud * 0.72);
  col = mix(col, uAcc, pow(cloud, 1.35) * 0.58 + core * 0.35);
  col += uAcc * core * 0.1;
...
void main(){
  ...
  vec3 col = mix(uBg, uMid, smoothstep(.2, .7, v));
  col = mix(col, uAcc, smoothstep(.55, 1., v + pulse) * .55);
  col += uAcc * pulse * .08;
...
void main(){
  ...
  vec3 col = uBg;
  col = mix(col, uMid, line * .55);
  col = mix(col, uAcc, line * glow + glow * .18);
...
void main(){
  ...
  vec3 col = uBg;
  col = mix(col, uMid, field * .7);
  col = mix(col, uAcc, field * field * .6 + rim * .4);
  col += uAcc * exp(-dm * 4.5) * .06;
